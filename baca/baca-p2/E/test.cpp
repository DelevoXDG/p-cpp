...
TEST(Branch, Branch) {
	// GTEST_SKIP();
	Branch b;
	EXPECT_EQ(branchInfo(b), std::string("0.0.0.0"));
	for (int i = 0; i < 8; i++) {
		b.growthBranch();
...
TEST(Garden, Garden) {
	...
	Tree* cloned = garden.getTreePointer(0);
	cloned->growthTree();
	cloned->growthTree();
	cloned->growthTree();
